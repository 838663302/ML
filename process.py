...
def build_dataset(sentences, word2id):
    indexed = [[word2id.get(token, 0) for token in jieba.lcut(sentence)] for sentence in sentences]
    dataset = []
    for data in indexed:
...
def process():
    ...
    train_dataset = build_dataset(train_data, word2id)
    test_dataset = build_dataset(test_data, word2id)
