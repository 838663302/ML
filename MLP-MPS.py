...
# 优先使用 MPS (Apple Silicon), 然后是 CUDA, 最后是 CPU
...
print(f"使用设备: {device}")
# MTCNN 使用 CPU，因为 MPS 的 adaptive_avg_pool2d 有兼容性问题
...
def train(embeddings, labels, label_map, epochs=30, batch_size=32, lr=0.001):
    ...
    # 加载最佳模型，输出分类报告
    model.load_state_dict(torch.load("mlp_model.pth", map_location=device, weights_only=True))
    model.eval()
    all_preds = []
    all_labels = []
    with torch.no_grad():
